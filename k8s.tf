    ...
    template {
      ...
      spec {
        container {
          name    = "kubectl"
          image   = var.kubectl_image
          command = ["kubectl --kubeconfig /etc/rancher/k3s/k3s.yaml -n cattle-system exec $(kubectl --kubeconfig /etc/rancher/k3s/k3s.yaml -n cattle-system get pods -l app=rancher | grep '1/1' | head -1 | awk '{ print $1 }') -- reset-password >> /tmp/admin_password.txt" ]
        }
        host_network                    = true
        automount_service_account_token = true
        service_account_name            = kubernetes_service_account.rancher_installer.metadata[0].name
